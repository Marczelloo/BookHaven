...
exports.getBookPage = async (req, res) => {
  ...
  try {
    const book = await BookService.getBookWithPromotion(bookId);
    ...
    let relatedBooks = await BookService.findRelatedBooks(book.genre, book._id);
    relatedBooks = await PromotionService.enhanceBooksWithPromotions(relatedBooks);
