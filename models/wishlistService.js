...
class WishlistService {
  ...
  static async moveToCart(session, bookIds) {
    let wishlist = session.wishlist || [];
    let cart = session.cart || [];

    // Find items to move
    const itemsToMove = wishlist.filter((item) => bookIds.includes(item.bookId.toString()));

    // Add items to cart (merge if already exists)
    for (const item of itemsToMove) {
      const existingCartIndex = cart.findIndex((cartItem) => cartItem.bookId.toString() === item.bookId.toString());
      if (existingCartIndex > -1) {
        cart[existingCartIndex].quantity += item.quantity;
      } else {
        cart.push({ bookId: item.bookId, quantity: item.quantity });
      }
    }

    // Remove items from wishlist
    wishlist = wishlist.filter((item) => !bookIds.includes(item.bookId.toString()));

    // Update session
    session.wishlist = wishlist;
    session.cart = cart;

    // Calculate new totals for remaining wishlist
    if (wishlist.length === 0) {
      return { wishlistItems: [], subtotal: 0, discount: 0, total: 0 };
    }

    const remainingBookIds = wishlist.map((item) => item.bookId);
    const books = await Book.find({ _id: { $in: remainingBookIds } });
    ...
    const discountPercent = subtotal > 100 ? 10 : subtotal > 200 ? 20 : 0;
    const discount = (discountPercent / 100) * subtotal;
    const total = wishlistItems.length === 0 ? 0 : 5 + subtotal - discount;
